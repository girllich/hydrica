...
public final class GenCave {
  ...
  // thanks to
  // http://github.com/tosik/BitboardCA/blob/master/src/OuterTotalisticCA.cpp
  // thanks to
  ...
  public static interface evaluator {
    public long eval(long s0, long s1, long s2, long s3, long s4, long s5,
                     long s6, long s7, long s8, long s9);
  }
  ...
  public static final evaluator widen = new evaluator() {
    public final long eval(long s0, long s1, long s2, long s3, long s4, long s5,
                           long s6, long s7, long s8, long s9) {
  ...
  public static final class noisegenimpl implements noisegen {
    ...
    public long noise(int x, int y, int z, float density) {
      return new BitsWithProbability(
                 density,
                 GenCave.getRandom(x + offsetx, y + offsety, z + offsetz))
          .nextLong();
    }
    public long noise(int x, int y, int z) {
      return new BitsWithProbability(
  ...
  public void initnoise(
      int level, java.util.Random r,
      int p) { // http://stackoverflow.com/questions/2075912/generate-a-random-binary-number-with-a-variable-proportion-of-1-bits
    for (int j = 8; j < 72; j++)
      for (int i = 8; i < 72; i++)
  ...
  public final void step(int level, int newlevel, evaluator e) {
    for (int x = 1; x < (board[level].length - 1); x++)
      for (int y = 1; y < (board[level][0].length - 1); y++) {
        ...
        // board[newlevel][x][y] = (~board[level][x][y] & (s0 | s1 | s6 | s7 |
        // s8 )) |
  ...
  public void cave3d(int x, int y, int z) {
    noisegen g = new noisegenimpl(x >> 3, y >> 3, z); // z is leveled
                                                      // differently
    initnoise2(4, g);
    ...
    // VolumetricSpaceArray vsa=new VolumetricSpaceArray(new
    // Vec3D(1,1,1),returnx,returny,24); vsa.getData()[117]=1f;
